...
(function(){
	...
	for (var i = 0; i < es.length; i++) {
		if (es[i].href === "/item") {				// Change all /item links to valid battle.net links
			var name = es[i].textContent.replace(' ', '-').toLowerCase();
			es[i].href = "https://us.battle.net/d3/en/item/" + name;
		} else if (es[i].href === "/skill") {	// Change all /skill links to valid battle.net links
			var name = es[i].textContent.toLowerCase();
			if (skills[name])
				es[i].href = "https://us.battle.net/d3/en/class/" + skills[name];
		}
	}

	es = document.getElementsByClassName("flair");
	for (var i = 0; i < es.length; i++) {
		if (es[i].textContent === "")
			continue;
		var r;
		if (es[i].className.indexOf("americas") !== -1)
			r = "us";
		else if (es[i].className.indexOf("europe") !== -1)
			r = "eu";
		else if (es[i].className.indexOf("asia") !== -1)
			r = "as";
		else
			continue;
